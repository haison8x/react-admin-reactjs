...
import { CreateDto, UpdateDto } from '../dto/invoice';
import { InvoiceService } from '../providers';
...
export class InvoiceController {
  constructor(private service: InvoiceService, private responseService: RetailResponseService) {}

  @Get('') // http://localhost:3000/api/invoices?filter={"has_ordered":true}&range=[0,99]&sort=["first_seen","DESC"]
  public async readAll(@RetailQuery() retailQuery: RetailQueryType, @Res() res: Response): Promise<void> {
    ...
    const invoices = await this.service.readMany(retailQuery);
    this.responseService.includeContentRange(res, 'invoices', retailQuery.range, count);

    res.status(200).json(invoices);
  }

  @Get(':id') // GET http://localhost:3000/api/invoices/:id
  public async read(@Param('id', ParseIntPipe) id: number): Promise<Invoice> {
  ...
  @Post() // POST http://localhost:3000/api/invoices
  ...
  @Put(':id') // PUT http://localhost:3000/api/invoices/:id
  ...
  @Put() // PUT http://localhost:3000/api/invoices/:id
  ...
  @Delete(':id') // DELETE http://localhost:3000/api/invoices/:id
  ...
  @Delete() // DELETE http://localhost:3000/api/invoices/:id
